...
type EncodeOptions struct {
	...
	PCMOutput        bool             // Raw PCM output (s16le, 48khz, stereo) - skips internal opus encoding
	RawInput         bool             // Raw PCM input (s16le, 48khz, stereo) - adds input args for ffmpeg
	...
	// The ffmpeg audio filters to use, see https://ffmpeg.org/ffmpeg-filters.html#Audio-Filters for more info Leave empty to use no filters. Warning: This string is passed directly to ffmpeg, so ensure it comes from a trusted source to prevent command injection or other issues.
...
func (opts *EncodeOptions) Validate() error {
	...
	if len(opts.AudioFilter) > 1024 {
		return errors.New("Audio filter too long (max 1024 chars)")
	}
	if strings.ContainsAny(opts.AudioFilter, "\r\n") {
		return errors.New("Audio filter cannot contain newlines")
	}
...
var StdEncodeOptions = &EncodeOptions{
	...
	CompressionLevel: 3, // Lowered from 10 for performance
	PacketLoss:       1,
...
type EncodeSession struct {
	...
	// buffer that stores unread bytes (not full frames) used to implement io.Reader
...
func (e *EncodeSession) run() {
	...
	// If reading from a pipe (stdin), we cannot "reconnect". Reconnect flags often cause ffmpeg to buffer extensively until it feels safe. We only use them if we are NOT using a pipe (i.e. we have a file path / URL).

	isPipe := e.pipeReader != nil || inFile == "pipe:0" || inFile == "-"

	if isPipe {
		// For pipes, we want low latency. These flags must be before -i to apply to the input (for analyzeduration/probesize) or are global checks.
	...
	if e.options.RawInput {
		args = append(args, "-f", "s16le", "-ac", "2", "-ar", "48000")
	}
	...
	args = append(args,
		...
		"-filter:a", fmt.Sprintf("volume=%.2f", float64(e.options.Volume)/256.0),
	...
	if e.options.PCMOutput {
		// Override for PCM output Remove -acodec libopus, -f ogg, -vbr, -application, -packet_loss, -frame_duration Add -f s16le -acodec pcm_s16le

		// Rebuild args for PCM
		args = []string{
			"-stats",
		}
	...
	// logln(ffmpeg.Args)
	fmt.Printf("[DCA-DEBUG] Running FFmpeg: %v\n", args)
...
func (e *EncodeSession) Cleanup() {
	...
	for _ = range e.frameChannel {
		// empty till closed empty till closed
...
func (e *EncodeSession) readStdoutPCM(stdout io.ReadCloser) {
	// Read raw PCM 16-bit little-endian samples Frame size = FrameLength * Channels * BytesPerSample 960 * 2 * 2 = 3840 bytes for 20ms

	frameSize := e.options.PCMFrameLen() * 2 // PCMFrameLen returns number of samples, * 2 bytes (16-bit)
	...
	for {
		...
		if err != nil {
			if err != io.EOF && err != io.ErrUnexpectedEOF {
				logln("Error reading ffmpeg stdout (PCM):", err)
			}
			break
		}

		// Copy buffer to avoid race conditions/reuse
		frameData := make([]byte, len(buf))
		copy(frameData, buf)
		e.frameChannel <- &Frame{frameData, false}
	}
}
