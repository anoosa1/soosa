...
func (s *StreamingSession) stream() {
	...
	// Create a reused timer for the loop
	timer := time.NewTimer(time.Second)
	// Stop it initially so we can reset it when needed.
	if !timer.Stop() {
		select {
		case <-timer.C:
		default:
...
func (s *StreamingSession) readNext(timer *time.Timer) error {
...
// Running returns whether the stream is running or not.
func (s *StreamingSession) Running() bool {
	s.Lock()
	defer s.Unlock()
	return s.running
}
